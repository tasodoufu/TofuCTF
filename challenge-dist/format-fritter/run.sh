#!/bin/sh
set -eu
image=tofuctf/format-fritter:local
container=tofuctf-format-fritter
volume=tofuctf-format-fritter-flag
port="${PORT:-31337}"
slug=format-fritter
command -v docker >/dev/null 2>&1 || { echo "Docker is required." >&2; exit 1; }
case "${1:-start}" in
  start) ;;
  stop) docker rm -f "$container" >/dev/null 2>&1 || true; docker volume rm "$volume" >/dev/null 2>&1 || true; echo "Format Fritter stopped."; exit 0 ;;
...
echo "Format Fritter is ready: nc 127.0.0.1 $port"

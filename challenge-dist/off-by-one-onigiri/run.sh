#!/bin/sh
set -eu
image=tofuctf/off-by-one-onigiri:local
container=tofuctf-off-by-one-onigiri
volume=tofuctf-off-by-one-onigiri-flag
port="${PORT:-31337}"
slug=off-by-one-onigiri
command -v docker >/dev/null 2>&1 || { echo "Docker is required." >&2; exit 1; }
case "${1:-start}" in
  start) ;;
  stop) docker rm -f "$container" >/dev/null 2>&1 || true; docker volume rm "$volume" >/dev/null 2>&1 || true; echo "Off-by-One Onigiri stopped."; exit 0 ;;
...
echo "Off-by-One Onigiri is ready: nc 127.0.0.1 $port"

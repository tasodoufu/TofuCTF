#!/bin/sh
set -eu
image=tofuctf/got-gnocchi:local
container=tofuctf-got-gnocchi
volume=tofuctf-got-gnocchi-flag
port="${PORT:-31337}"
slug=got-gnocchi
command -v docker >/dev/null 2>&1 || { echo "Docker is required." >&2; exit 1; }
case "${1:-start}" in
  start) ;;
  stop) docker rm -f "$container" >/dev/null 2>&1 || true; docker volume rm "$volume" >/dev/null 2>&1 || true; echo "GOT Gnocchi stopped."; exit 0 ;;
...
echo "GOT Gnocchi is ready: nc 127.0.0.1 $port"

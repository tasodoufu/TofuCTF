#!/bin/sh
set -eu
image=tofuctf/pivot-pierogi:local
container=tofuctf-pivot-pierogi
volume=tofuctf-pivot-pierogi-flag
port="${PORT:-31337}"
slug=pivot-pierogi
command -v docker >/dev/null 2>&1 || { echo "Docker is required." >&2; exit 1; }
case "${1:-start}" in
  start) ;;
  stop) docker rm -f "$container" >/dev/null 2>&1 || true; docker volume rm "$volume" >/dev/null 2>&1 || true; echo "Pivot Pierogi stopped."; exit 0 ;;
...
echo "Pivot Pierogi is ready: nc 127.0.0.1 $port"

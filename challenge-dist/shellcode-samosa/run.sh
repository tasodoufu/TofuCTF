#!/bin/sh
set -eu
image=tofuctf/shellcode-samosa:local
container=tofuctf-shellcode-samosa
volume=tofuctf-shellcode-samosa-flag
port="${PORT:-31337}"
slug=shellcode-samosa
command -v docker >/dev/null 2>&1 || { echo "Docker is required." >&2; exit 1; }
case "${1:-start}" in
  start) ;;
  stop) docker rm -f "$container" >/dev/null 2>&1 || true; docker volume rm "$volume" >/dev/null 2>&1 || true; echo "Shellcode Samosa stopped."; exit 0 ;;
...
echo "Shellcode Samosa is ready: nc 127.0.0.1 $port"

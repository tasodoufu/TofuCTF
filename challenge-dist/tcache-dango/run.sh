#!/bin/sh
set -eu
image=tofuctf/tcache-dango:local
container=tofuctf-tcache-dango
volume=tofuctf-tcache-dango-flag
port="${PORT:-31337}"
slug=tcache-dango
command -v docker >/dev/null 2>&1 || { echo "Docker is required." >&2; exit 1; }
case "${1:-start}" in
  start) ;;
  stop) docker rm -f "$container" >/dev/null 2>&1 || true; docker volume rm "$volume" >/dev/null 2>&1 || true; echo "Tcache Dango stopped."; exit 0 ;;
...
echo "Tcache Dango is ready: nc 127.0.0.1 $port"

#!/bin/sh
set -eu
image=tofuctf/uaf-udon:local
container=tofuctf-uaf-udon
volume=tofuctf-uaf-udon-flag
port="${PORT:-31337}"
slug=uaf-udon
command -v docker >/dev/null 2>&1 || { echo "Docker is required." >&2; exit 1; }
case "${1:-start}" in
  start) ;;
  stop) docker rm -f "$container" >/dev/null 2>&1 || true; docker volume rm "$volume" >/dev/null 2>&1 || true; echo "UAF Udon stopped."; exit 0 ;;
...
echo "UAF Udon is ready: nc 127.0.0.1 $port"

...
document.addEventListener('click',async event=>{const button=event.target.closest('.copy-access');if(!button)return;const label=button.querySelector('i');try{await navigator.clipboard.writeText(button.dataset.command);label.textContent='COPIED ✓'}catch{label.textContent='COPY FAILED'}setTimeout(()=>label.textContent='COPY',1400)});

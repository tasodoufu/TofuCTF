...
const card=(c,compact=false)=>`<article class="challenge ${isSolved(c.id)?'solved':''} ${compact?'compact':''}" id="${compact?'archive-':''}${c.id}"><div class="challenge-main"><div class="tags"><span>PWN</span><span>${compact?c.topic.toUpperCase():'BEGINNER +1'}</span><span>${c.points} PTS</span></div><h3>${c.title}</h3><p>${c.subtitle}</p><dl><div><dt>ARCH</dt><dd>${c.arch}</dd></div><div><dt>MITIGATIONS</dt><dd>${c.mitigations}</dd></div><div><dt>ACCESS</dt><dd>${access(c)}</dd></div></dl></div><aside class="challenge-side"><div class="difficulty"><span>DIFFICULTY</span><b>${'●'.repeat(c.level)}${'○'.repeat(5-c.level)}</b></div><a class="download" href="${c.download}" download>${compact?'Download':'Download challenge'} <span>↓</span></a><button class="open-modal" data-id="${c.id}" type="button">${isSolved(c.id)?'Solved ✓':'Submit flag'}</button><small class="solve-state">${isSolved(c.id)?'✓ Solved locally':'Not solved yet'}</small></aside></article>`;
